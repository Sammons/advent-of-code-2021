...
fn run() -> Result<(), String> {
    ...
    let mut boards_that_won = HashSet::new();
    let mut winning_boards = vec![];
    let bingo_board: Option<u32> = input.called_numbers.iter().find_map(|num| {
        println!("Calling {}", num);
        ...
        bingos.iter().find_map(|(board_idx, (x,y))| {
            if boards[*board_idx].is_tile_in_bingo((x,y)) {
                if !boards_that_won.contains(board_idx) {
                  winning_boards.push((*board_idx, boards[*board_idx].tally_unmarked() * num));
    ...
    println!("Done! x {:?}", winning_boards.last());
    Ok(())
}
